...
public class DictIdentifier {
	...
	static DictIdentifier getForId(int id) {
		...
		DictIdentifier ident = identifiers.get(id);
		if(ident == null) {
	...
	public String getName() {
		if(name == null) name = OreDictionary.getOreName(id);
		return name;
	}
	
	public String getCategory() {
		if(category == null) category = Pattern.compile("[A-Z].*").matcher(getName()).replaceFirst("");
		return category;
	}
