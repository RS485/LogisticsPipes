...
class GuideBookContents private constructor(val lang: String, val pages: Int, val title: String) {
    ...
    fun getPage(index: Int): Page? {
        ...
        try {
            ...
            LogisticsPipes.logger.error("Couldn't find page $index for language '$lang'!")
        }
        return null
    }
        ...
        fun load(lang: String = MinecraftClient.getInstance().languageManager.language.code): GuideBookContents? {
            try {
                ...
                res.use {
                    ...
                    if (pages < 1) {
                        LogisticsPipes.logger.error("Guide book for language '$lang' has $pages pages?!")
                        return null
                    }
                ...
                if (lang != "en_us") {
                    LogisticsPipes.logger.error("Couldn't find book data for language '$lang'! Falling back to en_us…")
                    return load("en_us")
                } else {
                    LogisticsPipes.logger.error("Couldn't find book data for en_us! Something is horribly wrong…")
                }
            } catch (e: UnsupportedOperationException) {
                LogisticsPipes.logger.error("Invalid book.json for language '$lang'!", e)
            }
            return null
        }
