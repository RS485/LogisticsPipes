...
public class TextureMatrix {
	...
	public void refreshStates(CoreUnroutedPipe pipe) {
		if(textureIndex != pipe.getTextureIndex()) dirty = true;
		textureIndex = pipe.getTextureIndex();
		if(isRouted != pipe.isRoutedPipe()) dirty = true;
		isRouted = pipe.isRoutedPipe();
		if(isRouted) {
			CoreRoutedPipe cPipe = (CoreRoutedPipe) pipe;
			for(int i=0;i<6;i++) {
				if(isRoutedInDir[i] != cPipe.getRouter().isRoutedExit(ForgeDirection.getOrientation(i))) dirty = true;
			...
			if(pointedOrientation != cPipe.getPointedOrientation()) dirty = true;
