	...
	public SinkReply sinksItem(ItemStack item, int bestPriority,
		...
		for(int i= 0; i<9; i++)
		{
			...
			if(getMaterials(i) != null)
			{
				...
				if(sink.equals(wanted))
				{
					IInventoryUtil invUtil = SimpleServiceLocator.inventoryUtilFactory.getInventoryUtil(_invProvider.getRawInventory());
					int room = invUtil.roomForItem(sink);
					if(room == 0) continue;
					return new SinkReply(SinkReply.FixedPriority.ItemSink, 99, false, false, 0, room);
				}
			}
		}
		return null;
