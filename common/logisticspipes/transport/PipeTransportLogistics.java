...
public class PipeTransportLogistics {
	...
	public int injectItem(LPTravelingItem item, EnumFacing inputOrientation) {
		...
		if (MainProxy.isServer(container.getWorld())) {
			...
			item.output = UtilEnumFacing.UNKNOWN;
		}
	...
	protected void reverseItem(LPTravelingItemServer item) {
		...
		} else if (item.output == UtilEnumFacing.UNKNOWN) {
			dropItem(item);
			return;
		}
	...
	public EnumFacing resolveUnroutedDestination(LPTravelingItemServer data) {
		List<EnumFacing> dirs = new ArrayList<EnumFacing>(Arrays.asList(UtilUtilEnumFacing.VALID_DIRECTIONS));
	...
	public EnumFacing resolveRoutedDestination(LPTravelingItemServer data) {
		...
		if (value == UtilEnumFacing.UNKNOWN && !data.getDoNotBuffer() && data.getBufferCounter() < 5) {
		...
		if (value != UtilEnumFacing.UNKNOWN && !getRoutedPipe().getRouter().isRoutedExit(value)) {
			...
			if (MainProxy.isServer(getWorld())) {
				...
				if (arrivingItem.getAdditionalTargetInformation() instanceof ITargetSlotInformation) {
					...
					if (inv instanceof ISidedInventory) {
						inv = new SidedInventoryMinecraftAdapter((ISidedInventory) inv, UtilEnumFacing.UNKNOWN, false, utilEnumFacing);
					}
	...
	public boolean canPipeConnect(TileEntity tile, EnumFacing side) {
		if (isRouted) {
			if (tile instanceof ILogisticsPowerProvider || tile instanceof ISubSystemPowerProvider) {
				...
				if (ori != null && ori != UtilEnumFacing.UNKNOWN) {
	...
	protected boolean endReached(LPTravelingItem item) {
		return item.getPosition() >= ((item.output == UtilEnumFacing.UNKNOWN) ? 0.75F : 1.0F);
	}
	...
	public void handleItemPositionPacket(int travelId, EnumFacing input, EnumFacing output, float speed, float position) {
