...
public class GuiRequestTable extends LogisticsBaseGuiScreen implements IItemSearch, ISpecialItemRenderer, IDiskProvider {
	...
	protected final String _title = StringUtils.translate(this.PREFIX + "titleItems");
	...
	public void initGui() {
		...
		buttonList.clear();
		buttonList.add(hideWhileSmall.addChain(new GuiButton(0, right - 55, bottom - 25, 50, 20, StringUtils.translate(this.PREFIX + "buttonRequest")))); // Request
		...
		buttonList.add(hideWhileSmall.addChain(new SmallGuiButton(3, guiLeft + 210, bottom - 15, 46, 10, StringUtils.translate(this.PREFIX + "buttonRefresh")))); // Refresh
		buttonList.add(hideWhileSmall.addChain(new SmallGuiButton(13, guiLeft + 210, bottom - 28, 46, 10, StringUtils.translate(this.PREFIX + "buttonContent")))); // Component
		buttonList.add(hideWhileSmall.addChain(new SmallGuiButton(9, guiLeft + 210, bottom - 41, 46, 10, StringUtils.translate(this.PREFIX + "buttonBoth")))); // Both
		buttonList.add(hideWhileSmall.addChain(new SmallGuiButton(20, right - 116, bottom - 41, 26, 10, StringUtils.translate(this.PREFIX + "buttonSort")))); // [Both], Craft & Supply
		...
		buttonList.add(hideShowButton = new SmallGuiButton(17, guiLeft + 173, guiTop + 5, 36, 10, StringUtils.translate("buttonHide"))); // Hide
		buttonList.add(Macrobutton = new SmallGuiButton(18, right - 55, bottom - 60, 50, 10, StringUtils.translate("buttonDisk")));
	...
	public void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		if (showRequest) {
			...
			if (buttonList.get(9) instanceof GuiCheckBox && ((GuiCheckBox) buttonList.get(9)).getState()) {
				mc.fontRenderer.drawString(StringUtils.translate(this.PREFIX + "textPopup"), guiLeft + 225, bottom - 56, 0x404040);
			} else {
				mc.fontRenderer.drawString(StringUtils.translate(this.PREFIX + "textPopup"), guiLeft + 225, bottom - 56, Color.getValue(Color.GREY));
			}
		...
		mc.fontRenderer.drawString(StringUtils.translate(this.PREFIX + "buttonSort") + ":", guiLeft + 136, guiTop + 55, 0xffffff);
				...
				extentionControllerLeft.addExtention(new GuiExtention() {
					...
					public void renderForground(int left, int top) {
						...
						if (resource != null) {
							...
							s = StringUtils.translate(GuiRequestTable.PREFIX + "textList");
						}
						...
						if (isFullyExtended()) {
							if (localControlledButton == null || orderIdForButton != entry.getKey()) {
								...
								localControlledButton = new SmallGuiButton(100, guiLeft - 35, guiTop + 10, 30, 10, StringUtils.translate(GuiRequestTable.PREFIX + "buttonMore"));
							...
							String ident = StringUtils.translate(GuiRequestTable.PREFIX + "textUpperID") + ": " + Integer.toString(entry.getKey());
					...
					public void handleMouseOverAt(int xPos, int yPos) {
						if (isFullyExtended()) {
							ordererPosition.keySet().stream()
								...
								list.add(ChatColor.BLUE + StringUtils.translate(GuiRequestTable.PREFIX + "textRequestType") + ": " + ChatColor.YELLOW + order.getType().name());
								list.add(ChatColor.BLUE + StringUtils.translate(GuiRequestTable.PREFIX + "textSendRouterID") + ": " + ChatColor.YELLOW + order
										.getRouterId());
							...
							List<String> list = new ArrayList<>();
							list.add(ChatColor.BLUE + StringUtils.translate(GuiRequestTable.PREFIX + "textRequestID") + ChatColor.YELLOW + entry.getKey());
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 0 && itemDisplay.getSelectedItem() != null) {
			...
			hideShowButton.displayString = showRequest ? StringUtils.translate(this.PREFIX + "buttonHide") : StringUtils.translate(this.PREFIX + "buttonShow");
			for (GuiButton button : hideWhileSmall) {
	...
	public void handleSimulateAnswer(Collection<IResource> used, Collection<IResource> missing, ISubGuiControler control, EntityPlayer player) {
		...
		control.setSubGui(new GuiRequestPopup(_entityPlayer, StringUtils.translate(this.PREFIX + "textComponents") + ": ", used, StringUtils.translate(this.PREFIX + "textMissing") + ": ", missing));
	}
