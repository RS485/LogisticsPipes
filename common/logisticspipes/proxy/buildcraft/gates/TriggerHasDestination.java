			...
			if (((LPBCPipe) pipe).pipe.pipe instanceof CoreRoutedPipe) {
				for(IStatementParameter parameter: parameters) {
					if (parameter != null && parameter.getItemStack() != null) {
						...
						if (SimpleServiceLocator.logisticsManager.hasDestination(ItemIdentifier.get(item), false, ((CoreRoutedPipe) ((LPBCPipe) pipe).pipe.pipe).getRouter().getSimpleID(), new ArrayList<>()) != null) {
							return true;
						}
					}
				}
