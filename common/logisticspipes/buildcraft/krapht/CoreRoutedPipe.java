...
import logisticspipes.buildcraft.krapht.network.TilePacketWrapper;
...
import logisticspipes.buildcraft.logisticspipes.IAdjacentWorldAccess;
import logisticspipes.buildcraft.logisticspipes.IRoutedItem;
import logisticspipes.buildcraft.logisticspipes.ITrackStatistics;
import logisticspipes.buildcraft.logisticspipes.PipeTransportLayer;
import logisticspipes.buildcraft.logisticspipes.RouteLayer;
...
public abstract class CoreRoutedPipe extends Pipe implements IRequestItems, IAdjacentWorldAccess, ITrackStatistics, IWorldProvider {
	...
	public logisticspipes.buildcraft.krapht.network.PacketPayload getLogisticsNetworkPacket() {
		logisticspipes.buildcraft.krapht.network.PacketPayload payload = new TilePacketWrapper(new Class[] { container.getClass(), transport.getClass(), logic.getClass() }).toPayload(xCoord, yCoord, zCoord, new Object[] { container, transport, logic });
			...
			if (getLogisticsModule() != null && getLogisticsModule().getGuiHandlerID() != -1){
				if(MainProxy.isServer()) {
					...
					if(getLogisticsModule() instanceof ModuleItemSink) {
						PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.ITEM_SINK_STATUS, xCoord, yCoord, zCoord, ((ModuleItemSink)getLogisticsModule()).isDefaultRoute() ? 1 : 0).getPacket(), entityplayer);
					}
					if(getLogisticsModule() instanceof ModuleExtractor) {
						PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.EXTRACTOR_MODULE_RESPONSE, xCoord, yCoord, zCoord, ((ModuleExtractor)getLogisticsModule()).getSneakyOrientation().ordinal()).getPacket(), entityplayer);
					}
					return true;
				} else {
					return false;
