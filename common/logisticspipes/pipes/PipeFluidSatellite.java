...
public class PipeFluidSatellite extends FluidRoutedPipe implements IRequestFluid, IHeadUpDisplayRendererProvider, IChestContentReceiver {
	...
	public PipeFluidSatellite(int itemID) {
		super(new BaseLogicFluidSatellite(), itemID);
	}
	...
	public void sendFailed(FluidIdentifier liquid, Integer amount) {
		if(logic instanceof IRequireReliableFluidTransport) {
			((IRequireReliableFluidTransport)logic).liquidLost(liquid, amount);
		}
	}
...
//TODO 	MainProxy.sendPacketToServer(new PacketPipeInteger(NetworkConstants.HUD_STOP_WATCHING, getX(), getY(), getZ(), 1).getPacket());
	...
	public void playerStartWatching(EntityPlayer player, int mode) {
		if(mode == 1) {
			...
			final ModernPacket packet = PacketHandler.getPacket(SatPipeSetID.class).setSatID(((BaseLogicFluidSatellite)this.logic).satelliteId).setPosX(xCoord).setPosY(yCoord).setPosZ(zCoord);
