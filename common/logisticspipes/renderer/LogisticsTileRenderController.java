...
public class LogisticsTileRenderController {
	...
	private class LaserBeamDataClient extends LaserBeamData {
		...
		public LaserBeamDataClient(float length, int timeout, boolean reverse, EnumFacing dir, int color) {
			...
			entity = new PipeFXLaserPowerBeam(pipe.getWorld(), new LPPosition((TileEntity) pipe), length, dir, color, pipe).setReverse(reverse);
	...
	private class LaserBallDataClient extends LaserBallData {
		...
		public LaserBallDataClient(float length, int timeout, int color) {
			...
			entity = new PipeFXLaserPowerBall(pipe.getWorld(), new LPPosition((TileEntity) pipe), color, pipe);
			Minecraft.getMinecraft().effectRenderer.addEffect(entity);
		}
		...
		}
		{
			...
			while (iter.hasNext()) {
				...
				if (data.timeout < 0 || data.isDeadEntity()) {
					data.setDead();
					if (data.sendPacket()) {
						MainProxy.sendPacketToAllWatchingChunk(pipe.getX(), pipe.getZ(), MainProxy.getDimensionForWorld(pipe.getWorld()), PacketHandler.getPacket(PowerPacketLaser.class).setColor(key).setRenderBall(true).setDir(UtilEnumFacing.UNKNOWN).setRemove(true).setTilePos(pipe));
					}
					iter.remove();
				}
			}
