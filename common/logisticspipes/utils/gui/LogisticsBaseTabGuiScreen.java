...
public class LogisticsBaseTabGuiScreen extends LogisticsBaseGuiScreen {
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		if(par3 == 0 && par1 > guiLeft && par1 < guiLeft + 220 && par2 > guiTop && par2 < guiTop + 20) {
			...
			for(int i=0;i<tabList.size();i++) {
				if(current_Tab == i) {
					if(tabList.get(i).handleClick(par1, par2, par3)) return;
				}
			}
	...
	protected void keyTyped(char p_73869_1_, int p_73869_2_) {
		for(int i=0;i<tabList.size();i++) {
			if(current_Tab == i) {
				if(tabList.get(i).handleKey(p_73869_2_, p_73869_1_)) return;
			}
		}
