...
public class LPPeripheralProvider implements IPeripheralProvider {
	...
	public IPeripheral getPeripheral(World world, int x, int y, int z, int side) {
		...
		if (tile instanceof LogisticsTileGenericPipe) {
			...
			return new LPPeripheralTilePipeWrapper((LogisticsTileGenericPipe) tile, UtilEnumFacing.VALID_DIRECTIONS[side]);
		}
		if (tile instanceof LogisticsSolidTileEntity) {
			return new LPPeripheralTileSolidWrapper((LogisticsSolidTileEntity) tile, UtilEnumFacing.VALID_DIRECTIONS[side]);
		}
		return null;
	}
}
