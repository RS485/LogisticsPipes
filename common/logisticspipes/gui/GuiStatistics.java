...
public class GuiStatistics extends LogisticsBaseGuiScreen {
	...
	public void initGui() {
		...
		if(itemDisplay_1 == null) itemDisplay_1 = new ItemDisplay(null, fontRendererObj, this, null, guiLeft + 10, guiTop + 18, xSize - 20, ySize - 100, new int[]{1,10,64,64}, true);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int mouse_x, int mouse_y) {
		...
		itemRender.renderItemAndEffectIntoGUI(fontRendererObj, getMC().renderEngine, stack, guiLeft + 31, guiTop + 3);
		...
		if(current_Tab == 0) {
			...
			if(itemDisplay_1.getSelectedItem() != null) {
				...
				if(task != null) {
					...
					GL11.glEnable(GL11.GL_DEPTH_TEST);
					RenderHelper.enableGUIStandardItemLighting();
					itemRender.renderItemAndEffectIntoGUI(fontRendererObj, getMC().renderEngine, task.item.makeNormalStack(1), guiLeft + 11, guiTop + 100);
					GL11.glDisable(GL11.GL_LIGHTING);
					GL11.glDisable(GL11.GL_DEPTH_TEST);
					itemRender.zLevel = 0.0F;
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		if(par3 == 0 && par1 > guiLeft && par1 < guiLeft + 220 && par2 > guiTop && par2 < guiTop + 20) {
			par1 -= guiLeft + 3;
			this.current_Tab = Math.max(0, Math.min(par1 / 25, TAB_COUNT - 1));
		} else {
			if(current_Tab == 0) {
	...
	public void handlePacket_1(List<ItemIdentifierStack> identList) {
		if(this.hasSubGui() && this.getSubGui() instanceof GuiAddTracking) {
			((GuiAddTracking)this.getSubGui()).handlePacket(identList);
