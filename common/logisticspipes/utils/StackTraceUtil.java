...
public class StackTraceUtil {
	...
	private static LinkedList<Pair<StackTraceElement,String>> getList() {
		LinkedList<Pair<StackTraceElement,String>> list = informationMap.get(Thread.currentThread());
		if(list == null) {
			list = new LinkedList<Pair<StackTraceElement,String>>();
			informationMap.put(Thread.currentThread(), list);
		}
		return list;
	}
	
	public static Info addTraceInformation(final String information, Info... infos) {
		if(!LPConstants.DEBUG) return new DummyInfo();
		StackTraceElement[] trace = Thread.currentThread().getStackTrace();
		final StackTraceElement calledFrom = trace[2];
		return addTraceInformationFor(calledFrom, information, infos);
	}
	...
	private static Info addTraceInformationFor(final StackTraceElement calledFrom, final String information, final Info... infos) {
		synchronized(informationMap) {
			getList().addLast(new Pair<StackTraceElement, String>(calledFrom, information));
			return new Info() {
				@Override
				public void end() {
					synchronized(informationMap) {
						if(getList().isEmpty()) {
							...
							Pair<StackTraceElement, String> pair = getList().getLast();
	...
	public static void printTrace() {
		synchronized(System.err) {
			synchronized(informationMap) {
				...
				while(current != null) {
					Iterator<Pair<StackTraceElement, String>> iter = getList().iterator();
					if(lastFound != null) {
						while(iter.hasNext()) {
					...
					while(iter.hasNext()) {
						Pair<StackTraceElement, String> pair = iter.next();
						if(compare(current, pair.getValue1())) {
							lastFound = pair;
