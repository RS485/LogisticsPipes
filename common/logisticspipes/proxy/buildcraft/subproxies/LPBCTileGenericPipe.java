...
public class LPBCTileGenericPipe extends TileGenericPipe implements IBCTilePart {
	...
	public Map<EnumFacing, List<StatementSlot>> activeActions = new HashMap<EnumFacing, List<StatementSlot>>();
		...
		if (attachPluggables) {
			...
			for (int i = 0; i < UtilEnumFacing.VALID_DIRECTIONS.length; i++) {
				if (pluggables[i] != null) {
					...
					pluggables[i].onAttachedPipe(this, UtilEnumFacing.getOrientation(i));
				}
			}
			notifyBlockChanged();
		}
