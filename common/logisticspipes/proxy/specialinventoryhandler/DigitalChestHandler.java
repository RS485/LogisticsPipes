...
public class DigitalChestHandler extends SpecialInventoryHandler {
	...
	public boolean isType(TileEntity tile) {
		if (DigitalChestHandler.apiIsBroken) {
			return false;
		}
		try {
			...
			DigitalChestHandler.apiIsBroken = true;
			return false;
		}
	}
	...
	public int itemCount(ItemIdentifier itemIdent) {
		...
		if (!content.getItem().equals(itemIdent)) {
			return 0;
		}
	...
	private ItemIdentifierStack getContents() {
		ItemStack[] data = _tile.getStoredItemData();
		if (data == null || data.length < 1 || data[0] == null || data[0].getItem() == null) {
			return null;
		}
	...
	public ItemStack add(ItemStack stack, ForgeDirection from, boolean doAdd) {
		...
		if (stack.stackSize - st.stackSize == 0) {
			return st;
		}
