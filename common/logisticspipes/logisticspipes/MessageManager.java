...
//TODO 	MainProxy.sendPacketToPlayer(new PacketItems(NetworkConstants.MISSING_ITEMS, errors,true).getPacket(), (Player)player);
		MainProxy.sendPacketToPlayer(PacketHandler.getPacket(MissingItems.class).setItems(errors).setFlag(true).getPacket(), (Player)player);
	}
...
//TODO 	MainProxy.sendPacketToPlayer(new PacketSimulate(NetworkConstants.COMPONENT_LIST, used,missing).getPacket(), (Player)player);		
		MainProxy.sendPacketToPlayer(PacketHandler.getPacket(ComponentList.class).setUsed(used).setMissing(missing).getPacket(), (Player)player);
	}
}
