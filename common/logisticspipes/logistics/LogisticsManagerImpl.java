...
public class LogisticsManagerImpl implements LogisticsManager {
	...
	public static final LogisticsManagerImpl INSTANCE = new LogisticsManagerImpl();

	private LogisticsManagerImpl() {}
	...
	public Tuple3<Integer, SinkReply, List<IFilter>> hasDestination(ItemIdentifier stack, boolean allowDefault, int sourceID, List<Integer> routerIDsToExclude) {
		...
		Tuple3<Integer, SinkReply, List<IFilter>> search = getBestReply(stack, sourceRouter, validDestinations, true, routerIDsToExclude, null, allowDefault);
	...
	private Tuple3<Integer, SinkReply, List<IFilter>> getBestReply(ItemIdentifier stack, Router sourceRouter, List<ExitRoute> validDestinations, boolean excludeSource, List<Integer> jamList, Tuple3<Integer, SinkReply, List<IFilter>> result, boolean allowDefault) {
		...
		for (ExitRoute candidateRouter : validDestinations) {
			...
			SinkReply reply = LogisticsManagerImpl.canSink(candidateRouter.destination, sourceRouter, excludeSource, stack, result.getValue2(), false, allowDefault);
	...
	public HashMap<ItemIdentifier, Integer> getAvailableItems(List<ExitRoute> validDestinations) {
		...
		for (ExitRoute r : validDestinations) {
			...
			if (!(r.destination.getPipe() instanceof ItemRequestProvider)) {
				continue;
			}
			...
			ItemRequestProvider provider = (ItemRequestProvider) r.destination.getPipe();
	...
	public LinkedList<ItemIdentifier> getCraftableItems(List<ExitRoute> validDestinations) {
		...
		for (ExitRoute r : validDestinations) {
			...
			if (!(r.destination.getPipe() instanceof ItemCrafter)) {
				continue;
			}
			for (IFilter filter : r.filters) {
			...
			ItemCrafter crafter = (ItemCrafter) r.destination.getPipe();
			List<ItemStack> craftedItems = crafter.getCraftedItems();
			if (craftedItems != null) {
				outer2:
				for (ItemStack craftedItem : craftedItems) {
