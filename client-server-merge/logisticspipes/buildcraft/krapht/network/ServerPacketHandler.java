...
import logisticspipes.buildcraft.krapht.routing.NormalOrdererRequests;
import logisticspipes.buildcraft.logisticspipes.macros.RequestHandler;
import logisticspipes.buildcraft.logisticspipes.macros.RequestHandler.RequestReply;
...
import logisticspipes.buildcraft.logisticspipes.modules.ModuleApiaristSink.FilterType;
import logisticspipes.buildcraft.logisticspipes.modules.ModuleExtractor;
...
import logisticspipes.buildcraft.logisticspipes.modules.SneakyOrientation;
...
public class ServerPacketHandler {
	...
	private static void onModuleGuiOpen(EntityPlayerMP player, PacketPipeInteger packet) {
		...
		if (cassiPipe.getLogisticsModule().getSubModule(packet.integer) instanceof ModuleItemSink) {
			PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.ITEM_SINK_STATUS, packet.posX, packet.posY, packet.posZ,
					(((ModuleItemSink) cassiPipe.getLogisticsModule().getSubModule(packet.integer)).isDefaultRoute() ? 1 : 0)).getPacket(), player);
		}
		if (cassiPipe.getLogisticsModule().getSubModule(packet.integer) instanceof ModuleExtractor) {
			PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.EXTRACTOR_MODULE_RESPONSE, packet.posX, packet.posY, packet.posZ,
					(((ModuleExtractor) cassiPipe.getLogisticsModule().getSubModule(packet.integer)).getSneakyOrientation().ordinal())).getPacket(), player);
		}
		if (cassiPipe.getLogisticsModule().getSubModule(packet.integer) instanceof ModuleProvider) {
			PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.PROVIDER_MODULE_INCLUDE_CONTENT, packet.posX, packet.posY, packet.posZ, (((ModuleProvider) cassiPipe.getLogisticsModule().getSubModule(packet.integer)).isExcludeFilter() ? 1 : 0)).getPacket(), player);
			PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.PROVIDER_MODULE_MODE_CONTENT, packet.posX, packet.posY, packet.posZ, (((ModuleProvider) cassiPipe.getLogisticsModule().getSubModule(packet.integer)).getExtractionMode().ordinal())).getPacket(), player);
		}
		if (cassiPipe.getLogisticsModule().getSubModule(packet.integer) instanceof ModuleAdvancedExtractor) {
			PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.ADVANCED_EXTRACTOR_MODULE_INCLUDED_RESPONSE, packet.posX, packet.posY, packet.posZ, (((ModuleAdvancedExtractor) cassiPipe.getLogisticsModule().getSubModule(packet.integer)).areItemsIncluded() ? 1 : 0)).getPacket(), player);
		}
	}
	...
	private static void onProviderModeChange(EntityPlayerMP player, PacketCoordinates packet) {
		...
		PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.PROVIDER_PIPE_MODE_CONTENT, packet.posX, packet.posY, packet.posZ, logic.getExtractionMode().ordinal()).getPacket(), player);
	}
	...
	private static void onProviderIncludeChange(EntityPlayerMP player, PacketCoordinates packet) {
		...
		PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.PROVIDER_PIPE_INCLUDE_CONTENT, packet.posX, packet.posY, packet.posZ, logic.isExcludeFilter() ? 1 : 0).getPacket(), player);
	}
	...
	private static void onSupplierModeChange(EntityPlayerMP player, PacketCoordinates packet) {
		...
		PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.SUPPLIER_PIPE_MODE_RESPONSE, packet.posX, packet.posY, packet.posZ, logic.isRequestingPartials() ? 1 : 0).getPacket(), player);
	}
	...
	private static void onExtractorModeChange(EntityPlayerMP player, PacketPipeInteger packet) {
		...
		if (slot <= 0) {
			if (piperouted.getLogisticsModule() instanceof ISneakyOrientationreceiver) {
				...
				PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.EXTRACTOR_MODULE_RESPONSE, packet.posX, packet.posY, packet.posZ, module.getSneakyOrientation().ordinal()).getPacket(), player);
				return;
			}
		} else {
	...
	private static void onProviderModuleModeChange(EntityPlayerMP player, PacketPipeInteger packet) {
		...
		if (slot <= 0) {
			if (piperouted.getLogisticsModule() instanceof ModuleProvider) {
				...
				PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.PROVIDER_MODULE_MODE_CONTENT, packet.posX, packet.posY, packet.posZ, module.getExtractionMode().ordinal()).getPacket(), player);
				return;
			}
		} else {
	...
	private static void onProviderModuleIncludeChange(EntityPlayerMP player, PacketPipeInteger packet) {
		...
		if (slot <= 0) {
			if (piperouted.getLogisticsModule() instanceof ModuleProvider) {
				...
				PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.PROVIDER_MODULE_INCLUDE_CONTENT, packet.posX, packet.posY, packet.posZ, module.isExcludeFilter() ? 1 : 0).getPacket(), player);
				return;
			}
		} else {
			if (piperouted.getLogisticsModule().getSubModule(slot - 1) instanceof ModuleProvider) {
	...
	private static void onAdvancedExtractorModuleIncludeChange(EntityPlayerMP player, PacketPipeInteger packet) {
		...
		if (slot <= 0) {
			if (piperouted.getLogisticsModule() instanceof ModuleAdvancedExtractor) {
				...
				PacketDispatcher.sendPacketToPlayer(new PacketPipeInteger(NetworkConstants.ADVANCED_EXTRACTOR_MODULE_INCLUDED_RESPONSE, packet.posX, packet.posY, packet.posZ, module.areItemsIncluded() ? 1 : 0).getPacket(), player);
				return;
			}
		} else {
	...
	private static void onCraftingPipeUpdateRequest(EntityPlayerMP player, PacketCoordinates packet) {
		...
		if(pipe.pipe instanceof PipeItemsCraftingLogistics) {
			if(pipe.pipe.logic instanceof BaseLogicCrafting) {
				...
				PacketDispatcher.sendPacketToPlayer(newpacket.getPacket(), player);
			}
		}
	}
	...
	private static void onDiskContentRequest(EntityPlayerMP player, PacketCoordinates packet) {
		...
		if(pipe.pipe instanceof PipeItemsRequestLogisticsMk2) {
			...
			PacketDispatcher.sendPacketToPlayer(new PacketItem(NetworkConstants.DISK_CONTENT, pipe.xCoord, pipe.yCoord, pipe.zCoord, ((PipeItemsRequestLogisticsMk2)pipe.pipe).getDisk()).getPacket(), player);
		}		
	}
		...
		if(pipe.pipe instanceof PipeItemsRequestLogisticsMk2) {
			...
			for(int i = 0;i < list.tagCount();i++) {
				if(i == packet.integer) {
					...
					PacketDispatcher.sendPacketToPlayer(new PacketItems(reply.items, !reply.suceed).getPacket(), player);
